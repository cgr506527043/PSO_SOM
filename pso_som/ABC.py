...
def get_center():
    centers = []
    data = np.asarray(som.dataset_old)
    winner = np.asarray(som.category)
    label = set(winner)

    for i in label:
        index = np.where(winner == i)
        centers.append(list(np.sum(data[index], axis=0) / len(data[index])))
    medoids_idx = []
    for center in centers:
        distances = []
        for i in data.tolist():
            distance = np.linalg.norm(np.array(i)-np.array(center), 2)
...
def rouletteWheelFunction(P):
    p_sorted_asc = sorted(P.items(), key = operator.itemgetter(1))      # 距离占比升序排列
    p_sorted_desc = dict(reversed(p_sorted_asc))            # 距离占比降序排列
...
databases = [{'filename': 'iris.data', 'has_id': False, 'class_position': 'last'}]
...
for database in databases:
    ...
    print("原始聚类中心:", centroids)
    # medoids_idx = get_center()
    # print("聚类中心索引:", medoids_idx)
    # datalist = som.dataset_old.tolist()
    # centers = []
    # for i in medoids_idx:
    #     centers.append(datalist[i])
    # print("聚类中心:", centers)
    # centroids = {}
    # for i in centers:
    #     centroids[som.classes_old.tolist()[som.dataset_old.tolist().index(i)]] = np.asarray(i)
    # print(centroids)

    ######## OUTPUT ########
    limits = [1,2,3]
    for limit in limits:
        best_soltions, new_centroids = ABC(trainning_set, trainning_set_classes, centroids.copy(), a_limit = limit, max_iter = 100)
